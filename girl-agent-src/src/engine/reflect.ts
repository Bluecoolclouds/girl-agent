...
newFacts — СТРОГИЕ ПРАВИЛА:
Записывай ТОЛЬКО биографические факты о нём, которые он назвал явно сам:
✅ Его имя/ник: "его зовут Дима"
✅ Город/страна: "живёт в Екатеринбурге"
✅ Работа/учёба: "работает программистом", "учится в универе"
✅ Возраст: "ему 24 года"
✅ Хобби/интересы которые он назвал: "играет в Valorant и CS"
✅ Конкретные личные факты: "у него есть кот", "не пьёт"

НЕ записывай:
❌ Обрывки фраз из переписки: "ниче", "есть", "да", "окей", любые слова < 4 слов
❌ Её ответы или её слова — только ФАКТЫ О НЁМ
❌ Предположения и догадки — только то что он сам сказал явно
❌ Эмоциональные оценки ("он груб", "он интересный") — их пиши в feelingShift
❌ Факты которые уже есть в existingFacts — не дублируй
Если фактов нет — верни newFacts: []`;
...
Верни JSON:
{
  "newFacts": ["строго биографические факты о НЁМ (имя, город, работа, возраст, хобби). Минимум 5 слов в записи. Формат: 'его зовут X', 'живёт в X', 'работает X'. Если фактов нет — пустой массив."],
  "feelingShift": "1-2 предложения о том как поменялось её отношение (или null если ничего не изменилось)",
  "stageHint": "оставить как есть | повысить | понизить | dumped"
    ...
    if (Array.isArray(parsed.newFacts) && parsed.newFacts.length) {
      // Фильтр: только строки с минимум 4 словами, похожие на биографический факт
      const filtered = (parsed.newFacts as unknown[])
        .filter((f): f is string => typeof f === "string" && f.trim().split(/\s+/).length >= 4);
      if (filtered.length) {
        await appendMd(cfg.slug, "memory/long-term.md",
          `\n\n## ${new Date().toISOString().slice(0, 16)}\n` + filtered.map((f: string) => `- ${f}`).join("\n"));
      }
    }
