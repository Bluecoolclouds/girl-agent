...
${ctx.recentEmojiReaction ? `\nКонтекст: он только что (< 5 минут назад) поставил реакцию ${ctx.recentEmojiReaction.emoji}${ctx.recentEmojiReaction.herMessageSnippet ? ` на твоё сообщение "${ctx.recentEmojiReaction.herMessageSnippet}"` : ""}. Реальная девушка иногда мимоходом упоминает это в ответе — легко, не делая из этого события: "ой спасибо за сердечко)", "рада что смешно)", "ну понятно что думаешь)". Не обязательно — по ощущению.` : ""}
