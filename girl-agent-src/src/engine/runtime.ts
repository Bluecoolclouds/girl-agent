...
export class Runtime extends EventEmitter {
  ...
  private async generateAndSend(
    ...
    const scopeHint = scope === "acquaintance"
      ? `\nЭто потенциальный подписчик/покупатель в личке. Не используй память/отношения основного парня — его нет в этом контексте. Флирт и романтические/сексуальные заходы — это нормально и нужно подыгрывать: отвечай игриво, с лёгкой интригой, намекай что есть закрытый контент. НЕ отшивай и не ставь жёстких границ на флирт — это твоя работа. Жёстко реагируй только на реальные угрозы, хамство и требования бесплатного. Если он намекает на что-то горячее — подхвати тему и мягко направь к платному контенту.${channelLink ? `\nЕСЛИ он ЯВНО говорит что готов купить прямо сейчас ("я готов купить", "куплю", "хочу купить", "готов платить", "давай куплю") — НЕ тяни и НЕ уклоняйся. Сразу дай ссылку на закрытый канал: ${channelLink} — там он и найдёт платный контент. Пример ответа: "окей) вот ссылка — ${channelLink} там всё есть". Клиент который хочет платить прямо сейчас — не упускай его.` : ""}`
      : "";
    const messages: ChatMessage[] = [
