...
export class Runtime extends EventEmitter {
  ...
  private async generateAndSend(
    ...
    const channelLinkHint = channelLink ? `\nЕСЛИ спрашивают ссылку на канал или "где найти" — дай сразу: ${channelLink}. Не спрашивай "зачем тебе" — это рабочий вопрос.` : "";
    const scopeHint = scope === "acquaintance"
      ? `\nЭто потенциальный подписчик/покупатель в личке. Не используй память/отношения основного парня — его нет в этом контексте. Флирт, заигрывание, просьбы показать фото — это нормально, отвечай игриво, с лёгкой интригой, намекай что есть закрытые фото. НЕ отшивай и не ставь жёстких границ — это твоя аудитория. Жёстко реагируй только на реальные угрозы, хамство и требования бесплатного. Если он просит фото — мягко направляй к платному контенту или используй [SEND_PHOTO].${channelLink ? `\nЕСЛИ он ЯВНО говорит что готов купить прямо сейчас ("я готов купить", "куплю", "хочу купить", "готов платить", "кидай", "скинь фото") — НЕ тяни. Используй [SEND_PHOTO] чтобы переслать фото — он увидит превью и сможет открыть за Stars. Если нет фото в пуле — дай ссылку на канал: ${channelLink}. Клиент который хочет платить — не упускай его.` : ""}${channelLinkHint}`
      : `\nТы контент-мейкер. Просьбы показать фото, "покаж себя", вопросы о контенте — это рабочие запросы, не домогательства. Отвечай открыто и игриво. Если просит обычное фото лица — используй [SEND_PHOTO:face] или [SEND_PHOTO:selfie] без лишних условий. Если медиатека пустая — направляй к каналу.${channelLinkHint}`;
    const messages: ChatMessage[] = [
