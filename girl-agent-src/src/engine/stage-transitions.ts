...
const SOFT_CLOSE_PATTERNS = [
  ...
  // TG-приватка (основной формат продажи)
...
const HARD_CLOSE_PATTERNS = [
  ...
  /давай куплю/i, /хочу приобрести/i,
  /куплю\b/i, /покупаю\b/i,
  // Беру — осторожно, только с контекстом (короткое слово)
  /\bберу\b/i,
  // Запрос доступа/ссылки — явный сигнал готовности
  /хочу доступ/i, /дай ссылку/i, /скинь ссылку/i, /есть ссылка/i,
  /ссылку на канал/i, /ссылку на приватк/i,
  /как вступить/i, /как попасть в канал/i, /хочу в канал/i,
  /добавь в канал/i, /добавь меня/i,
];
